...
public class PullRequestOpenedTest {
    ...
    @Test
    void titleEndsWithDot_failingTest() throws IOException {
        when().payloadFromClasspath("/pullrequest-opened-title-ends-with-dot.json")
                .event(GHEvent.PULL_REQUEST)
                .then().github(mocks -> {
            verify(mocks.pullRequest(527350930))
                    // This message is deliberately incorrect, to demonstrate a failing test
                    .comment("Thanks for your PULL REQUEST! Everything's good, nothing to say.");
            verifyNoMoreInteractions(mocks.ghObjects());
        });
    }
